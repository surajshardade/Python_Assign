...
def Speak_IACO(data,time1,time2):
    for index in range(len(data)):
        if data[index]!=' ':
            os.system("say "+ d[data[index]])  #install : sudo apt install gnustep-gui-runtime

            #os.system("espeak "+d[data[index]])

            time.sleep(time1)
        else :
            time.sleep(time2)

data=input("Enter string to speak  : ")
time1=float(input("Enter time to pause between ICAO words:"))
time2=float(input("Enter time to pause between words : "))

Speak_IACO(data,time1,time2)
